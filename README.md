...
Google Maps Platform replaced the old universal $200/month credit with **free
monthly calls per SKU**, tiered:
...
How this package's calls map to those SKUs:

- **`suggest()` → Autocomplete (New)** — an *Essentials* SKU. Pass a
  `sessionToken` option (terminated by a `details()` call) to bundle keystrokes
  into one session; without it, each request bills individually.
- **`details()` → Place Details (New)** — the SKU tier depends on the requested
  fields. This package's field mask includes `formattedAddress` and
  `addressComponents`, which puts it in the *Pro* tier (~5,000 free/month) — the
  binding constraint for typical usage.

**Cap your spend** in the Cloud Console:

- **Hard cap (actually stops requests):** *APIs & Services → Places API (New) →
  Quotas* → set *Requests per day*. Exceeding it returns HTTP 429, which the
  package surfaces as an `AddressLookupException` rather than a surprise bill.
- **Budget alerts (email only, do not stop usage):** *Billing → Budgets &
  alerts*.

> Per-1,000 rates and the field-tier boundaries change periodically — confirm
> current numbers on the [official pricing page](https://mapsplatform.google.com/pricing/)
> before budgeting.
